...
class DatabaseCompareService
{
    ...
    public function compareSchemas($baseConnection, $targetConnection, $tables = [])
    {
        ...
        foreach ($baseColumns as $col) {
            $default = strtoupper($col->COLUMN_DEFAULT) === 'NULL' ? null : $col->COLUMN_DEFAULT;
        ...
        foreach ($baseMap as $table => $columns) {
            ...
            if (!isset($targetMap[$table])) {
                ...
                foreach ($columns as $colName => $details) {
                    ...
                    if (!$isPrimary) {
                        if ($details['default'] !== null) {
                            $defaultSQL = "DEFAULT " . $details['default']; // removed quotes to fix enum issue
                ...
                if (!empty($primaryKeys)) $cols[] = "PRIMARY KEY (" . implode(',', $primaryKeys) . ")";
            ...
            foreach ($columns as $colName => $details) {
                ...
                if (!$isPrimary) {
                    if ($details['default'] !== null) {
                        $defaultSQL = "DEFAULT " . $details['default']; // removed quotes
                    } elseif ($details['nullable'] === 'YES') {
                        $defaultSQL = "DEFAULT NULL";
                    }
                }
